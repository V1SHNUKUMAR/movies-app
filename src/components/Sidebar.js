...
const Sidebar = (props) => {
  ...
  const highlightTab = (tabId = "tab0") => {
    ...
    while (i < tabs.length) {
      ...
      if (tabs[i].id !== tabId) {
        tabs[i].style = "border-left: none;color: rgba(255, 255, 255, 0.5)";
      }
      i++;
    }
    window.scrollTo(0, 0);
      ...
      >
        <Link to={"/"}>
          ...
          <div id="logo" className="flex gap-2 items-center px-10">
            ...
            <h1 className="text-2xl sm:text-xl xl:text-2xl font-bold text-violet-400">
              Cinemix
            </h1>
          </div>
        </Link>
        ...
        <ul id="tabs" className="space-y-6 text-lg text-white/50 md:text-base">
          {categories.map((category, index) => (
            <li
              id={"tab" + index}
              key={index}
              className="tab px-12 py-0.5 duration-300 cursor-pointer hover:text-violet-500 hover:border-l-2 hover:border-violet-500"
              onClick={() => {
                setIsMenuOpen(false);
